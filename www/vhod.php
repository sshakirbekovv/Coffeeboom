...
mysql_select_db("erkebulan", $connect);
$provn = mysql_query("SELECT name AS _name FROM users where password = '".$password."'");
$row = mysql_fetch_assoc($provn);
$_SESSION['$name']=$name;
$provp = mysql_query("SELECT password AS _password FROM users where name = '".$name."'");
$ro = mysql_fetch_assoc($provp);
if($row['_name']==$name && $ro['_password']==$password){
  echo '<script>location.replace("reg.php");</script>';
} else {
  echo '<script>location.replace("notf.php");</script>';
}
